...
| **MessageSources** | `src/features/chat/MessageSources.tsx` | Composant sobre (< 100 lignes) d'affichage des sources de recherche citées sous les réponses assistant (titre tronqué, domaine, liens `_blank` sécurisés `noopener noreferrer`, repli au-delà de 3 sources, cibles ≥ 24 px) [À VALIDER] | `ChatMessageItem.tsx` |
