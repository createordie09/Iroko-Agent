...
| **ClaudeChat** | `src/features/chat/ClaudeChat.tsx` | Vue conversationnelle unique (assemblage modulaire < 400 lignes : conteneur défilant, messages virtualisés via ChatMessageList, suivi streaming accessible, inspecteur latéral, modales) | `ZyriconAppShell.tsx` |
| **ChatMessageList** | `src/features/chat/ChatMessageList.tsx` | Conteneur de messages avec virtualisation réactive $O(\log N)$, index accessible pour la recherche navigateur (`hidden="until-found"` et `beforematch`), compensation anti-saut et navigation clavier | `ClaudeChat.tsx` |
| **ChatMessageItem** | `src/features/chat/ChatMessageItem.tsx` | Bulle de message unitaire (`<article>`, `<h3>` masqué, actions au survol/focus, puces de pièces jointes, artéfacts intégrés) | `ChatMessageList.tsx` |
